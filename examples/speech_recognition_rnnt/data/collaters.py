...
class Seq2SeqCollater(object):
    ...
    def collate(self, samples):
        ...
        bsz_one =torch.ones([bsz,1]).long()
        bsz_one = bsz_one*3
        prev_output_tokens_rnnt = torch.cat([bsz_one,prev_output_tokens[:,1:]],dim=1)

        if prev_output_tokens is not None:
            batch["net_input"]["prev_output_tokens"] = prev_output_tokens_rnnt
        return batch
