...
class AsrDataset_Kaldi(FairseqDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        path = self.aud_paths[index].replace('.wav','.npy')
        if not os.path.exists(path):
            raise FileNotFoundError("Audio file not found: {}".format(path))
        
        vid_data = self.load_video(index)
        output_cmvn = np.load(path)
        output_cmvn = torch.from_numpy(output_cmvn)
        output_cmvn = output_cmvn.squeeze(0).permute(1,0)
