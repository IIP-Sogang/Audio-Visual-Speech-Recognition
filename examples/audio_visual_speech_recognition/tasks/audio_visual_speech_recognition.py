...
from examples.audio_visual_speech_recognition.data import AsrDataset, AsrDataset_video, AsrDataset_torch_mel, AsrDataset_Kaldi
...
def get_asr_dataset_from_json(data_json_path, tgt_dict, dataset_method=1, video_offset=0):
    ...
    with open(data_json_path, "rb") as f:
        ...
        if dataset_method == 2:
            return AsrDataset_video(
                aud_paths, vid_paths, frame_sizes, tgt, tgt_dict, ids, speakers, video_offset=video_offset
            )
        if dataset_method == 3:
            return AsrDataset_torch_mel(
                aud_paths, vid_paths, frame_sizes, tgt, tgt_dict, ids, speakers, video_offset=video_offset
            )
        if dataset_method == 4:
            return AsrDataset_Kaldi(
                aud_paths, vid_paths, frame_sizes, tgt, tgt_dict, ids, speakers, video_offset=video_offset
            )
...
class AVSpeechRecognitionTask(FairseqTask):
    ...
    def load_dataset(self, split,combine=False, video_offset=0, **kwargs):
        ...
        self.datasets[split] = get_asr_dataset_from_json(
            data_json_path, self.tgt_dict, dataset_method=self.args.dataset_method, video_offset=video_offset)
