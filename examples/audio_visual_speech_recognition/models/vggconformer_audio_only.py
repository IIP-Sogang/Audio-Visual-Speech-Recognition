...
@register_model("avsr_vggconformer_audio_only")
...
class VGGLSTMDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        if conv_config is not None:
            if conv_config[-1][0] != lstm_config[0][0]:
                self.layers.append(Linear(conv_config[-1][0], lstm_config[0][0]))
...
@register_model_architecture("avsr_vggconformer_audio_only", "BiModalvggconformer_avsr_audio_only_base")
...
#    args.conv_dec_config = 'None'
    args.lstm_dec_config = getattr(
        args, "lstm_dec_config", "((256,320,0.1),) * 1"
    )
