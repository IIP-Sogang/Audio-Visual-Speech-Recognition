...
@register_model("avsr_vggconformer_video_only")
class VGGTransformerModel_video_only(BaseFairseqModel):
    """
    ...
    def forward(self, audio_src_tokens, audio_src_lengths, video_src_tokens, video_src_lengths, prev_output_tokens, **kwargs):
        ...
        decoder_out = self.decoder(prev_output_tokens, encoder_out=video_encoder_out, **kwargs)
        return decoder_out
    
    def forward_decoder(self, prev_output_tokens, audio_encoder_outs, video_encoder_outs, incremental_state, **kwargs):
        return self.decoder(prev_output_tokens, video_encoder_outs, incremental_state, **kwargs)
    ...
    def extract_features(self, audio_src_tokens, audio_src_lengths, video_src_tokens, video_src_lengths, prev_output_tokens, **kwargs):
        audio_encoder_out = None
        video_encoder_out = self.video_encoder(video_src_tokens, src_lengths=video_src_lengths, **kwargs)
        features = self.decoder.extract_features(prev_output_tokens, audio_encoder_out=video_encoder_out, **kwargs)
        return features
    ...
    def max_positions(self):
        return (self.video_encoder.max_positions(), 1e6, self.decoder.max_positions())
...
@register_model_architecture("avsr_vggconformer_video_only", "BiModalvggconformer_avsr_video_only_base")
def BiModalvggtransformer_avsr_video_only_base(args):
...
#    args.video_vggblock_enc_config = 'None'
    args.conformer_enc_config = getattr(
        ...
        # args, "conformer_enc_config", "((256, 64, 4, 4, 2, 32, 0.1,0.1,0.1,256),) * 12"
