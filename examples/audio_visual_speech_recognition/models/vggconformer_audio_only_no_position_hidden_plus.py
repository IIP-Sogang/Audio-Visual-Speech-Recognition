...
@register_model("avsr_vggconformer_audio_only_no_position_hidden_plus")
...
class simpleLSTM(nn.Module):
    ...
    def forward(self,x,encoder_out):
        ...
        time, bsz, emb = x.size()
        #pdb.set_trace()
        h0 = encoder_out[-1,:,:].unsqueeze(0) # T B C -> 1 B C
        
        x = torch.cat(
            [x, h0.expand(time,bsz,-1)],
            dim=2,
        )
        initial_state = (
            h0,  # hidden
            torch.zeros_like(h0),  # cell
        )
        
        y ,_ = self.lstm(x,initial_state)
        return y
...
class VGGLSTMDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.fc_out = Linear(lstm_config[-1][1], vocab_size)
...
@register_model_architecture("avsr_vggconformer_audio_only_no_position_hidden_plus", "BiModalvggconformer_avsr_audio_only_no_position_hidden_plus_base")
...
#    args.video_vggblock_enc_config = 'None'
    ...
    args.audio_enc_output_dim = getattr(args, "audio_enc_output_dim", 320)
    args.video_enc_output_dim = getattr(args, "video_enc_output_dim", 512)
    args.tgt_embed_dim = getattr(args, "tgt_embed_dim", 512)
    #args.conv_dec_config = getattr(args, "conv_dec_config", "((256, 3, True),) * 4")
#    args.conv_dec_config = 'None'
    args.lstm_dec_config = getattr(
        args, "lstm_dec_config", "((832,320,0.1),) * 1"
    )
