...
@register_model("e2e_avse_avsr_vggtransformer")
class e2e_avse_avsr_VGGTransformerModel(BaseFairseqModel):
    """
    ...
    def __init__(self,avse,avsr):
        ...
        aa = torch.load("avse_model/new_AVSE.pth")
        aa_key = list(aa.keys())
        # pdb.set_trace()
        aaa_key = aa_key[:822] + aa_key[1522:]
        # self.unet.load_state_dict(torch.load("avse_model/new_AVSE.pth"))
        # pdb.set_trace()
        # aaa_key = list(self.unet.keys())
        new_key = {}
        for i in range(len(aaa_key)):
            new_key[str(aaa_key[i])] = aa[str(aaa_key[i])]

        self.unet.load_state_dict(new_key)
   
        # pdb.set_trace()
        self.avsr = avsr
        self.avsr.load_state_dict(torch.load("result/audio_visual_speech_recognition/BiModalvggtransformer_avsr_base/model/new_feature_VGG_base_wi_transformer_CE_yh/checkpoint39.pt")['model'])
    ...
    def forward(self, audio_src_tokens, audio_src_lengths, video_src_tokens, video_src_lengths, prev_output_tokens, **kwargs):
        ...
        decoder_out = self.avsr.decoder(prev_output_tokens, audio_encoder_out=audio_encoder_out, video_encoder_out=video_encoder_out, **kwargs)
        
        return decoder_out
    
    def forward_avse_encoder(self, audio_src_tokens, video_src_tokens, **kwargs):
        # pdb.set_trace()
        avse_out = self.unet(video_src_tokens,audio_src_tokens.permute(0,2,1))
        avse_out = avse_out.permute(0,2,1)
    ...
    def forward_decoder(self, prev_output_tokens, audio_encoder_outs, video_encoder_outs, incremental_state, **kwargs):
        # pdb.set_trace()
        return self.avsr.decoder(prev_output_tokens, audio_encoder_outs, video_encoder_outs, incremental_state, **kwargs)
...
@register_model_architecture("e2e_avse_avsr_vggtransformer", "e2e_BiModalvggtransformer_avse_avsr_base")
