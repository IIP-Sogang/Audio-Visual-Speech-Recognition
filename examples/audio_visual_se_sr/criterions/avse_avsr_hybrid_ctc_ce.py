...
from examples.audio_visual_se_sr.data.data_utils import encoder_padding_mask_to_lengths
from examples.audio_visual_se_sr.utils.wer_utils import Code, EditDistance, Token


logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)

@register_criterion("avse_avsr_hybrid")
class avse_avsr_hybrid(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, log_probs=True):
        ...
        avse_enc_output = model.forward_avse_encoder(**sample["net_input"])
        clean_src = sample["clean_src_tokens"]
        # pdb.set_trace()
        criterion_avse = nn.L1Loss()
        loss_avse = criterion_avse(avse_enc_output,clean_src)
        ...
        loss_avsr = self.args.hybrid_ctc_alpha * loss_ctc + (1 - self.args.hybrid_ctc_alpha) * loss_ce
        
        loss = self.args.avse_avsr_ce_alpha * loss_avse + (1-self.args.avse_avsr_ce_alpha) * loss_avsr
