...
@register_criterion("avse_avsr_cross_entropy_acc")
class avse_avsr_cross_entropy_acc(FairseqCriterion):
    ...
    def forward(self, model, sample, reduction="sum", log_probs=True):
        ...
        enc_output = model.forward_avse_encoder(**sample["net_input"])
        clean_src = sample["clean_src_tokens"]
        # pdb.set_trace()
        criterion_avse = nn.L1Loss()
        loss_avse = criterion_avse(enc_output,clean_src)

        target = model.get_targets(sample, net_output)


        lprobs, loss_avsr = self.compute_loss(
