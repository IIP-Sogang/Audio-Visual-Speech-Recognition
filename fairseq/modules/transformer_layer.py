...
#        )
        ...
        if self.audio_encoder_attn is not None and self.video_encoder_attn is not None:
            residual = x
            audio_x = self.maybe_layer_norm(self.encoder_attn_layer_norm, x, before=True)
            video_x = audio_x
            if prev_attn_state is not None:
                if incremental_state is None:
                    incremental_state = {}
                ...
                if len(prev_attn_state) >= 3:
                    saved_state["prev_key_padding_mask"] = prev_attn_state[2]
                self.audio_encoder_attn._set_input_buffer(incremental_state, saved_state)
                self.video_encoder_attn._set_input_buffer(incremental_state, saved_state)
...
class JW_TransformerBiModalDecoderLayer(nn.Module):
    """Bi-Modal Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False):
        ...
        # pdb.set_trace()
        if no_encoder_attn:
            self.av_encoder_attn = None
            self.encoder_attn_layer_norm = None
        else:
            self.av_encoder_attn = MultiheadAttention(
                self.embed_dim,
                args.decoder_attention_heads,
                kdim=getattr(args, 'encoder_embed_dim', None),
                vdim=getattr(args, 'encoder_embed_dim', None),
        ...
        self.fusion = nn.Sequential(
            Linear(self.embed_dim*2, self.embed_dim*4),
            LayerNorm(self.embed_dim*4, export=export),
            nn.ReLU(),
    ...
    def forward(
        ...
        video_encoder_out1 = video_encoder_out.permute(1,2,0) # T B F --> B F T

        # pdb.set_trace()
        # video_new_encoder_out = F.interpolate(input=video_encoder_out1,scale_factor=audio_encoder_out.size(0)/video_encoder_out.size(0)).permute(2,0,1)
        video_new_encoder_out = F.interpolate(input=video_encoder_out1,size=(audio_encoder_out.size(0))).permute(2,0,1)
        # print(audio_encoder_out.shape, video_new_encoder_out.shape)
        av_encoder_out = torch.cat((audio_encoder_out,video_new_encoder_out), dim=2) # T B 2F
        av_encoder_out1 = self.fusion(av_encoder_out)
        if self.av_encoder_attn is not None:
            residual = x
            x = self.maybe_layer_norm(self.encoder_attn_layer_norm, x, before=True)
            if prev_attn_state is not None:
                if incremental_state is None:
                ...
                if len(prev_attn_state) >= 3:
                    saved_state["prev_key_padding_mask"] = prev_attn_state[2]
                self.av_encoder_attn._set_input_buffer(incremental_state, saved_state)

            
            x, av_attn = self.av_encoder_attn(
                query=x,
                key=av_encoder_out1,
                value=av_encoder_out1,
                key_padding_mask=None,
                incremental_state=incremental_state,
                static_kv=True,
                need_weights=need_attn or (not self.training and self.need_attn),
    ...
    def forward(
        ...
        if self.audio_encoder_attn is not None and self.video_encoder_attn is not None:
            ...
            modality_coeff = self.MA_softmax(
                    torch.cat(
                        (audio_coeff, video_coeff), 2
                        ) # idx 0 - audio, idx 1 - video
                    )
