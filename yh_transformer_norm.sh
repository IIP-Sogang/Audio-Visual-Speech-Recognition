...
MAX_EPOCH=75
NUM_WORKERS=20
ARCH=BiModalvggtransformer_avsr_norm_DCM_base
CODE=new_featur_VGG_base_wi_norm_DCM_hybrid_alpha_01
...
if [ $1 -eq 1 ]; then
	CUDA_VISIBLE_DEVICES=$IDX_GPU python train.py \
		...
		--user-dir $USER_DIR \
		--fp16 \
		--dataset-method 3 # 1 = original / 2 = video early / 3 = torch mel
		#--no-save
